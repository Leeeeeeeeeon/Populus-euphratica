IE.est1 <- function(dat1,dat2,parin2,interval=c(1,10)){
  ...
  for(i in n1:n2){
    ...
    ndat$pheno<-y11
    ndat1$pheno<-y22
    h01 <- try(IE.H0(ndat,ndat1,parin2),TRUE)
    if (class(h01) == "try-error") 
      h01 <- NA
    h02 <- try(IE.H1(y11,y22,SNP1,init.par=h01,times),TRUE)
    if (class(h02) == "try-error") 
      h02 <- NA
    LR <- 2*(h01[1]-h02[1])

    if(is.na(h01)||is.na(h02)){
      allpar <- c(LR,rep(NA,32))
    }else{
      allpar <- c(LR,h02)
    }
...
IE.H1 <- function(y11,y22,SNP1,init.par,times){
  ...
  for(i in 1:length(snp.type)){
    ...
    #if(SNP.p[1]==0)
    #  SNP.p[1]<-0.1
    r1 <- optim(init.par[7:16],ind.mle,s.y=SNP.p,s.t=times,x1=SNP.p[1],x2=SNP.p[tlength+1],
